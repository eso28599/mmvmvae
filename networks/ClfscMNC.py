...
class ResnetBlockFC(nn.Module):
    """Residual block for fully-connected (vector) data."""
    def __init__(self, fin, fout, fhidden=None, is_bias=True):
        ...
        self.fc0 = nn.Linear(self.fin, self.fhidden, bias=is_bias)
...
class ClfscMNC_FC(nn.Module):
    ...
    def __init__(self, n_features, n_classes=10, hidden_dim=128, nblocks=3):
        super().__init__()
        layers = [ResnetBlockFC(n_features, hidden_dim)]
        for _ in range(nblocks - 1):
            layers.append(ResnetBlockFC(hidden_dim, hidden_dim))
